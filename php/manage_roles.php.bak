...
<!--php code: login-->
	...
	if(isset($_POST['xusername'])){$xusername=$_POST['xusername'];}
	if(isset($_POST['xpassword'])){$xpassword=$_POST['xpassword'];}
	if(isset($_POST['loginstatus'])){$xloginstatus=$_POST['loginstatus'];}
	...
	if (isset($_POST['button_disco'])){
		$xusername=''; $xpassword=''; $xloginstatus='red';
	}
	...
	// PATCHED: use $config['server_host'] instead of uninitialised $host
...
if (isset($_POST['button_addrole'])){
	$role=$_POST['textaddrole']; $status=$_POST['textaddstatus'];
	$contact=$_POST['textaddcontact']; $notes=$_POST['textaddnotes'];
	...
	if ($conn->query($sqltext)===TRUE){$sqlstatus='successful';}
	else {$sqlstatus='failed '.$conn->error.'...'.$sqltext;}
}
...
if (isset($_POST['button_editrole'])){
	$role=$_POST['textnewrole']; $status=$_POST['textnewstatus'];
	$contact=$_POST['textnewcontact']; $notes=$_POST['textnewnotes'];
...
$conn=new mysqli($host,$accessun,$accesspw,$dbname);
if ($conn->connect_error){ echo '<h2 class="centertext"> please connect to the database </h2>'; exit; }

$sqltext="Select * from `".$dbname."`.`list_cage_role_assignments`;";
$results=$conn->query($sqltext);

$currrole=isset($_POST['role_selection']) ? $_POST['role_selection'] : "";
$currstatus=''; $curractive=''; $currcontact=''; $currnotes='';
...
<body>
	<div id="header">
		<form id="loginbox" action="" method="post">
			...
			<table style="color:white;font-size:10px;position:absolute;top:0px;right:60px;">
				<tr><th>user:</th><th><input type="text" name="xusername" value="<?php echo $xusername; ?>" style="width:100px;font-size:10px;" /></th></tr>
				<tr><td>pass:</td><td><input type="password" name="xpassword" value="<?php echo $xpassword; ?>" style="width:100px;font-size:10px;" /></td></tr>
			</table>
	...
	<div id="left_navmenu">
		...
		<form action="../php/cagerole.php" method=post target="_blank">
			...
			<input type=hidden name="dbname" value="<?php echo $_POST['dbname']; ?>" />
			<input type=hidden name="button_login" value="connect" />
			<input type=submit class="button" name="" value="Cage Role Manager" />
		</form>
	...
	<form id="role_selection_form" method=post>
		...
		<h3>Add Role:</h3>
		<table>
			<tr><th>Role</th><th>Status Options</th><th>Main Contact</th><th>Notes</th></tr>
			<tr>
			...
			<tr><td colspan="4"><input type=submit name="button_addrole"></td></tr>
		</table>
		...
		function showRole(newValue){ document.getElementById("role_selection_form").submit(); }
		</script>
		<table>
			<tr><th>Role</th><th>Status Options</th><th>Main Contact</th><th>Notes</th></tr>
			<tr>
			...
			<tr><td><input type=submit name="button_editrole"></td></tr>
		</table>
	</form>
	</div>
	</div>
