...
// posted variables
if(isset($_POST['line_filter'])){$line_filter=$_POST['line_filter'];}
else {$line_filter='all';}

if(isset($_POST['gender_filter'])){$gender_filter=$_POST['gender_filter'];}
else {$gender_filter='all';}

if(isset($_POST['source_category_selection'])){$source_category_selection=$_POST['source_category_selection'];}
else {$source_category_selection='all';}

if(isset($_POST['category_selection'])){$category_selection=$_POST['category_selection'];}
else {$category_selection='';}

if(isset($_POST['cage_selection'])){
	if(is_array($_POST['cage_selection'])){$cage_selection=$_POST['cage_selection'];}
	else {$cage_selection=array($_POST['cage_selection']);}
} else {$cage_selection=array('');}

if(isset($_POST['cagelist_selection'])){$cagelist_selection=$_POST['cagelist_selection'];}
else {$cagelist_selection='';}

if(isset($_POST['roleA_selection'])){$roleA_selection=$_POST['roleA_selection'];}
else {$roleA_selection='all';}

if(isset($_POST['roleB_selection'])){$roleB_selection=$_POST['roleB_selection'];}
else {$roleB_selection='Limbo';}
...
if ($row===$gender_filter){$gender_listbox.='<option value="'.$row.'" selected>'.$row.'</option>';}
else {$gender_listbox.='<option value="'.$row.'" >'.$row.'</option>';}
}
...
if ($row===$source_category_selection){$source_category_listbox.='<option value="'.$row.'" selected>'.$row.'</option>';}
else {$source_category_listbox.='<option value="'.$row.'" >'.$row.'</option>';}
}
...
if($row['line']===$line_filter){$line_listbox .= '<option value="'.$row["line"].'" selected>'.$row["line"].'</option>';}
else {$line_listbox .= '<option value="'.$row["line"].'">'.$row["line"].'</option>';}
}
$line_listbox .= '</select>';
$conn->close();
...
while($row=mysqli_fetch_array($results)){
if ($row['cageid']===$cagelist_selection){$cage_listbox.='<option value="'.$row['cageid'].'" selected>'.$row['cageid'].'</option>';}
else{$cage_listbox.='<option value="'.$row['cageid'].'">'.$row['cageid'].'</option>';}
}
$cage_listbox.='</select>';
$conn->close();
...
$conn=new mysqli($host,$accessun,$accesspw,$dbname);
if ($line_filter==="all" or $line_filter===null){$lf='';}
else {$lf='`line`="'.$line_filter.'" and ';}

if ($gender_filter==="all" or $gender_filter===null){$gf='';}
else {$gf='`gender`="'.$gender_filter.'" and ';}

if ($source_category_selection==="all" or $source_category_selection===null){$sf='';}
else {$sf='left(`currentcage`,1)=left("'.$source_category_selection.'",1) and ';}

if ($roleA_selection==="all" or $roleA_selection===null){$locf='';}
elseif ($roleA_selection==="unknown"){$locf='(`cagerole_assignment` is null or `cagerole_assignment`="unknown") and ';}
else {$locf='`cagerole_assignment`="'.$roleA_selection.'" and ';}

$sql_where_text=substr($lf.$gf.$sf.$locf,0,-4);
if (strlen($sql_where_text)>0){$sql_where_text=' and '.$sql_where_text;}
...
while($row=mysqli_fetch_array($results)){
	...
	if ($nkey!==false){$sourcecage_listbox.='<option value="'.$row['currentcage'].'" selected>'.$row['currentcage'].'</option>';}
	else{$sourcecage_listbox.='<option value="'.$row['currentcage'].'">'.$row['currentcage'].'</option>';}
...
<body>
			...
			<div id="left_navmenu">
					 ...
					 <input type=hidden name="dbname" value="<?php echo $_POST['dbname']; ?>" />
					 <input type=hidden name="button_login" value="connect" />
					 <input type=submit class="button" name="" value="Export Animal Info" />
					 </form>
			</div>
...
function submitForm(){ document.getElementById("cage_selection_form").submit(); }
</script>
			<table>
			<tr>
			<th>Line Filter:</th><th>Gender Filter:</th>
			<th>Source Cage Category:</th><th>Current Role:</th>
			</tr>
			<tr>
			...
			<table>
			<tr><th>Current Cages (select to move):</th></tr>
			<tr><td><?php echo $sourcecage_listbox; ?></td></tr>
			</table>
			<table>
			<tr><td><input type=submit id="addcage_single" name="addcage_single" value="&#8595; Assign Selected Cages to Role Below"></td></tr>
			<tr><th>Destination Role:</th></tr>
			<tr><td><?php echo $locB_listbox;?></td></tr>
			<tr><th>Cages already with this role:</th></tr>
			<tr><td colspan=2><?php echo $cage_listbox; ?></td></tr>
			</table>
