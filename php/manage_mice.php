...
<td ><input class="tinylistbox" type=hidden name="mouseautono'.$ck.'" id="mouseautono'.$ck.'" readonly="readonly" value='.$ck.' >
	</td>
...
foreach ($arrayman as $man) {
	...
	//data_comments
	if($comments[$man]!=""){
		$sqltext.="INSERT INTO `data_comments` (`mouseautono`,`commentdate`,`general_comment`) VALUES (".$man.",curdate(),'".$comments[$man]."');";
	}
	//table_genotypes
	foreach (range(0,$genecount-1,1) as $i){
		if($genearray[$i][$man]!=""){
			$sqltext.="UPDATE `table_genotypes` SET `allele`='".$genearray[$i][$man]."' WHERE `mouseautono`=".$man." and `allelegroup`='".$genelist[$i]."';";
		}
	}
...
$sqltext="SELECT table_mice.mouseautono as 'man',line,idno,gender,dob,dod,currentcage FROM `table_mice` where ".$sql_where_text." ORDER BY `line` asc, `mouseautono` asc;";
//echo $sqltext;
$results=$conn->query($sqltext);
$mice_results=$results;
...
<head>
			...
			<title>Manage Mice - <?php echo $dbname; ?></title>
...
<!--CONTENT SECTION-->
			...
			<form id="mice_management_form" name="allele_management_form" method=post>
